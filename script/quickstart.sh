...
ln -s ${WORKSROOT}/config/neovim/init.vim "${XDG_DATA_HOME:-$HOME}"/.config/nvim/init.vim
ln -s ${WORKSROOT}/config/neovim/coc-settings.json "${XDG_DATA_HOME:-$HOME}"/.config/nvim/coc-settings.json
ln -s ${WORKSROOT}/config/neovim/lua "${XDG_DATA_HOME:-$HOME}"/.config/nvim/lua
ln -s ${WORKSROOT}/config/git/git-commit-template.txt "${XDG_DATA_HOME:-$HOME}"/.git-commit-template.txt
ln -s ${WORKSROOT}/config/git/.gitconfig "${XDG_DATA_HOME:-$HOME}"/.gitconfig

echo "################################################################################"
echo "# vim-plug"
echo "################################################################################"
cp ${WORKSROOT}/config/neovim/plug.vim "${XDG_DATA_HOME:-$HOME/.local/share}"/nvim/site/autoload

if [[ "$OSTYPE" == "linux-gnu"* ]]; then

	echo "################################################################################"
	echo "# Apt Source Update"
	echo "################################################################################"
	...
	cp ${WORKSROOT}/third-party/im-select /usr/local/bin/im-select
	...
	echo "# IM-SELEC 输入法自动切换"
	# https://github.com/daipeihust/im-select/tree/master/macOS/out/intel
	cp ${WORKSROOT}/third-party/im-select.macos /usr/local/bin/im-select
	chmod 777 /usr/local/bin/im-select
